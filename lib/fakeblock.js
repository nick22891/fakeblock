...
Fakeblock.prototype.canUserAccessMethod = function (methodName) {
    ...
    var hasAccess = accessList.indexOf(role) >= 0;

    // if don't have access and is a partner, check for permissions
    if (!hasAccess && role === 'partner') {
        for (var i = 0; i < permissions.length; i++) {
            if (accessList.indexOf('partner:' + permissions[i]) >= 0) {
                return true;
            }
        }
    }
...
Fakeblock.prototype.applyAcl = function (data, methodName, dataType) {
    ...
    if (!_.isEqual(data, dataCopy)) {
        console.log('Fakeblock blocked some data for name=' + this.name +
